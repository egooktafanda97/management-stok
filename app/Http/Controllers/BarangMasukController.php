...
#[Controllers()]
#[Group(prefix: 'barangmasuk',  middleware: ['web',  'auth:web', 'role:gudang'])]
class BarangMasukController extends Controller
{
    ...
    public function index()
    {
        return view('Page.BarangMasuk.index', [
            'barangMasuk' => $this->barangMasukService->getBarangMasuk()
        ]);
    }
    ...
    {
        return response()->json($this->barangMasukService->getBarangMasukToday());
    }
    ...
    public function formtambah()
    {
        return view('Page.BarangMasuk.tambah', [
            ...
            "satuan_beli" => $this->jenisSatuanService->getByAgency(),
        ]);
    }

    #[Get("produk/{id}/satuan")]
    #[RestController("api", ["auth:api"])]
    ...
    {
        try {
            $pushData =  $this->barangMasukService->fromDTOs((new BarangMasukDTOs(
    ...
    {
        try {
            $cencel = $this->barangMasukService->cancel($id);
            Alert::success('Success', 'Barang masuk berhasil dibatalkan!');
            return redirect()->back();
        } catch (\Throwable $th) {
            Alert::error('Error', 'Gagal membatalkan barang masuk: ' . $th->getMessage());
            return redirect()->back();
        }
    }
