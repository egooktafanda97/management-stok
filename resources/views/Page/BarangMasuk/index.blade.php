                ...
                <div class="ms-auto">
                    <div class="btn-group">
                        <a class="btn btn-success" href="{{ url('barangmasuk/tambah') }}">
                            <i class='bx bx-plus-circle'></i> Tambah Data Barang Masuk
                        </a>
                    </div>
                </div>
            </div>
                            ...
                            <tbody>
                                @foreach ($barangMasuk as $barangMasuk)
                                    <tr>
                                        ...
                                        <td>{{ $barangMasuk->produk->name }}</td>
                                        <td>{{ $barangMasuk->supplier->name ?? '' }}</td>
                                        <td>{{ 'Rp. ' . number_format($barangMasuk->harga_beli) }} /
                                            {{ $barangMasuk->satuanBeli->name }}</td>
                                        <td>{{ $barangMasuk->jumlah_sebelumnya }}
                                            {{ $barangMasuk->satuanSebelumnya->name ?? '' }}
                                        </td>
                                        <td>{{ $barangMasuk->jumlah_barang_masuk }}
                                            {{ $barangMasuk->satuanBeli->name }}
                                        </td>

                                        <td>{{ (int) app(\App\Services\KonversiSatuanService::Class)->convertToSatuanStok(
                                            $barangMasuk->produk->id,
                                            $barangMasuk->satuanBeli->id,
                                            $barangMasuk->jumlah_barang_masuk,
                                        )['konversi'] ?? 0 }}
                                            {{ $barangMasuk->satuanSebelumnya->name ?? '' }}
                                        </td>
