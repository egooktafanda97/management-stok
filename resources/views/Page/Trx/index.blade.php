...
@section('content')
    <div class="page-wrapper" x-data="{
        ...
        exeevn: '0',
        payType: null,
        listPaymentType: [], // list payment type dari database payment_types
        addToProdOrder(prod) {
            if (this.prodOrder.find(item => item.id == prod.id) == undefined) {
                prod.qty = 1;
                console.log('addToProdOrder', prod);
                this.prodOrder.push(prod);
                this.totalOrder = this.prodOrder.reduce((acc, item) => acc + (item.priceTotal * item.qty), 0);
                this.searchQuery = '';
                this.prodSerach = [];
                this.orders.push({
                    id: prod.id,
                    qty: 1,
                    harga: prod.priceTotal,
                    satuan: prod.satuan_aktif
                });
            }
        },
        ...
        fetch('/api/trx/init-trx', {
            ...
            .then(data => {
                ...
                // findPaymentType('cash');
                payType = listPaymentType.find(item => (item.name == 'CASH' || item.name == 'cash')).id;
            })
                    ...
                    <div class="row">
                        <div class="col-md-8">
                                        ...
                                        <template :key="prod.id" x-for="prod in prodSerach">
                                            <div class="bg-gray-50 shadow-sm p-2 flex items-center hover:bg-gray-100 active:bg-gray-300"
                                                ...
                                                <div class="flex-1 pl-2 mr-16">
                                                    <div class="font-semibold text-sm" x-text="prod.name"></div>
                            ...
                            <div class=" p-2 rounded-md shadow-md m-1" x-show="prodOrder.length > 0">
                                <template :key="prod.id" x-for="prod in prodOrder">
                                    {{-- <pre x-text="JSON.stringify(prod, null, 2)"></pre> --}}
                                    <div class="flex items-center p-4 bg-white rounded-lg" x-data="{
                                        harga: prod?.priceTotal,
                                        harga_aktual: prod?.init_unit?.priece,
                                        pengurangan_diskon: prod?.total_diskon ?? 0,
                                        diskon: prod?.init_unit?.diskon ?? 0,
                                        satuan: prod?.init_unit?.jenis_satuan_jual_id ?? '',
                                        qty: prod.qty,
                                        total: prod.priceTotal * prod.qty,
                                        updateSatuanNharga(val) {
                                            const hargaItem = prod.unit_prieces.find(item => item.jenis_satuan_jual_id == val);
                                            this.harga = hargaItem.priece - (hargaItem.priece * (this.diskon / 100));
                                            this.harga_aktual = hargaItem.priece;
                                            this.diskon = hargaItem.diskon;
                                            this.satuan = hargaItem.jenis_satuan_jual_id;
                                        ...
                                        <div class="flex-1 flex flex-col">
                                            <span class="font-semibold text-md text-gray-800" x-text="prod.name"></span>
                                            <div class="flex items-center">
                                                <div class="flex flex-col">
                                                    ...
                                                    <del class="text-sm text-gray-500 mr-5"
                                                        x-text="formatRupiah(harga_aktual)"></del>
                                                </div>
                                                <select
                                                    ...
                                                    <template :key="hargaItem.id" x-for="hargaItem in prod.unit_prieces">
                                                        <option :selected="hargaItem.jenis_satuan_jual_id == satuan"
                                                            :value="hargaItem.jenis_satuan_jual_id" x-text="hargaItem.name">
                                                        </option>
                                                    </template>
                                                </select>
                                            </div>
                        ...
                        <div class="col-md-4">
                            <div class="p-2 rounded-md shadow-md m-1">
                                <div class="max-w-lg w-full mb-5" x-data="{
                                    searchQueryUser: '',
                                    listUser: [],
                                    loading: false,
                                    userSearch() {
                                    ...
                                    <div class="form-group mb-3 relative">
                                        <label class="form-label font-semibold" for="exampleFormControlInput1">
                                            Pelanggan
                                        </label>
                                        <div class="flex items-center">
                                        ...
                                        <div class="absolute mt-20 z-50 inset-0" x-show="listUser.length > 0">
                                            <template :key="users.id" x-for="users in listUser">
                                                <div class="bg-gray-50 shadow-sm p-2 flex items-center hover:bg-gray-200 active:bg-gray-300"
                                                    style="cursor:pointer"
                                                    x-on:click="(()=>{
                                                    user =  users;
                                                    searchQueryUser = `${users.nama} - ${users.oncard_account_number}`;
                                                    listUser = [];
                                                })">
                                                    <div class="flex-1 pl-2 mr-16">
                                                        <div class="text-sm">
                                                            <div class="flex flex-col">
                                                                <div class="text-sm" x-text="users.nama"></div>
                                                                <div class="text-sm" x-text="users.oncard_account_number">
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                ...
                                <hr class="mb-2">
                                <div class="flex justify-between items-center mb-3" x-data="{ diskon_trx: 0 }">
                                    <div class="flex items-center">
                                    ...
                                    {{-- <h2 class="font-semibold text-lg" x-text="totalOrder">
                                        0</h2> --}}
                                </div>
                                ...
                                <div class="form-group mt-3" x-data="{
                                    ...
                                    <div class="flex flex-col items-center">
                                        <select class="form-control border border-gray-300 rounded-md py-2 px-3"
                                            x-model="payType">
                                            <option value="">Pilih Metode Pembayaran</option>
                                            <template :key="item.id" x-for="item in listPaymentType">
                                                <option :value="item.id" x-text="item.name"></option>
                                            </template>
                                        </select>
                                    </div>
                                    <div class="mt-3" x-show="payType == 1">
                                                ...
                                                swal({
                                                        ...
                                                        switch (value) {
                                                            case 'clear':
                                                                ...
                                                                break;
                                                            case 'print':
                                                            openPrintWindow(response.data?.data?.invoiceDTOs?.invoice_id?? '');
                                                            location.reload();
                                                                break;
                                                            default:
                                                                break;
